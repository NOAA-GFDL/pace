...
class GeosDycoreWrapper:
    ...
    def __init__(
        ...
        if gtfv3_single_rank_override >= 0:
            comm = ndsl.util.NullComm(gtfv3_single_rank_override, 6, 42)
        ...
        partitioner = ndsl.util.CubedSpherePartitioner(
            ndsl.util.TilePartitioner(self.layout)
        )
        self.communicator = ndsl.util.CubedSphereCommunicator(
            comm,
            partitioner,
            timer=self.perf_collector.timestep_timer,
        )

        sizer = ndsl.util.SubtileGridSizer.from_namelist(
        ...
        damping_coefficients = ndsl.util.grid.DampingCoefficients.new_from_metric_terms(
            metric_terms
        )
    ...
    def _put_fortran_data_in_dycore(
        ...
        ndsl.util.utils.safe_assign_array(state.u.view[:], u[isc:iec, jsc : jec + 1, :])
        ndsl.util.utils.safe_assign_array(state.v.view[:], v[isc : iec + 1, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(state.w.view[:], w[isc:iec, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(state.ua.view[:], ua[isc:iec, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(state.va.view[:], va[isc:iec, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(
        ...
        ndsl.util.utils.safe_assign_array(state.delz.view[:], delz[isc:iec, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(state.pt.view[:], pt[isc:iec, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(state.delp.view[:], delp[isc:iec, jsc:jec, :])

        ndsl.util.utils.safe_assign_array(state.mfxd.view[:], mfxd)
        ndsl.util.utils.safe_assign_array(state.mfyd.view[:], mfyd)
        ndsl.util.utils.safe_assign_array(state.cxd.view[:], cxd[:, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(state.cyd.view[:], cyd[isc:iec, :, :])

        ndsl.util.utils.safe_assign_array(state.ps.view[:], ps[isc:iec, jsc:jec])
        ndsl.util.utils.safe_assign_array(
            state.pe.data[isc - 1 : iec + 1, jsc - 1 : jec + 1, :], pe
        )
        ndsl.util.utils.safe_assign_array(state.pk.view[:], pk)
        ndsl.util.utils.safe_assign_array(state.peln.view[:], peln)
        ndsl.util.utils.safe_assign_array(state.pkz.view[:], pkz)
        ndsl.util.utils.safe_assign_array(state.phis.view[:], phis[isc:iec, jsc:jec])
        ndsl.util.utils.safe_assign_array(
            state.q_con.view[:], q_con[isc:iec, jsc:jec, :]
        )
        ndsl.util.utils.safe_assign_array(state.omga.view[:], omga[isc:iec, jsc:jec, :])
        ndsl.util.utils.safe_assign_array(
        ...
        ndsl.util.utils.safe_assign_array(state.qice.view[:], q[isc:iec, jsc:jec, :, 2])
        ndsl.util.utils.safe_assign_array(
        ...
        ndsl.util.utils.safe_assign_array(state.qcld.view[:], q[isc:iec, jsc:jec, :, 6])
